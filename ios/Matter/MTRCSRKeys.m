...
static const NSString * MTRIPKKeyChainLabel = @"esp.rainmaker.matter.IPK";
static const NSString * MTRCAKeyChainLabel = @"esp.rainmaker.matter.CA";
...
{
    CFErrorRef error = NULL;
    CFDataRef outData = SecKeyCreateSignature(_privateKey, kSecKeyAlgorithmECDSASignatureMessageX962SHA256, (__bridge CFDataRef) message, &error);
...
+(SecKeyRef)setPrivateKeyInKeychain:(SecKeyRef) encodedPrivateKey withGroupId:(NSString *)grpId {
    ...
    NSMutableDictionary * deleteQuery = [[NSMutableDictionary alloc] initWithDictionary:[MTRCSRKeys privateKeyParamsWithGroupId:grpId]];
...
+ (SecKeyRef)loadCAPrivateKeyWithGroupId:(NSString *)grpId {
    ...
    if (status != errSecSuccess || keyDataRef == nil) {
        NSLog(@"Did not find CA private key in the keychain for group: %@", grpId);
        return NULL;
    }

    NSLog(@"Found an existing CA private key in the keychain for group: %@", grpId);
    NSData * encodedKey = CFBridgingRelease(keyDataRef);

    NSData * keyData = [[NSData alloc] initWithBase64EncodedData:encodedKey options:0];
    if (keyData == nil) {
        NSLog(@"Could not base64-decode CA private key");
        return NULL;
    }
...
/// Public key params with group id
/// - Parameter groupId: group id
+ (NSDictionary *)publicKeyParamsWithGroupId:(NSString *)groupId
{
    NSString * finalKeyLabel = [[NSString alloc] initWithFormat:@"%@.%@.pub",MTRCAKeyChainLabel, groupId];
    return @{
        (__bridge NSString *) kSecClass : (__bridge NSString *) kSecClassKey,
        (__bridge NSString *) kSecAttrApplicationLabel : finalKeyLabel,
        (__bridge NSString *) kSecAttrKeyClass : (__bridge NSString *) kSecAttrKeyClassPublic,
...
+(SecKeyRef)setPublicKeyInKeychain:(SecKeyRef) encodedPublicKey withGroupId:(NSString *)grpId {
    ...
    NSData * keyData = (__bridge_transfer NSData *) SecKeyCopyExternalRepresentation(encodedPublicKey, &pubKeyError);
    if (pubKeyError) {
        NSLog(@"Could not get public key external representation: %@", (__bridge NSError *) pubKeyError);
        return NULL;
    }
    
    // Delete existing key first
    NSMutableDictionary * deleteQuery = [[NSMutableDictionary alloc] initWithDictionary:[MTRCSRKeys publicKeyParamsWithGroupId:grpId]];
    SecItemDelete((__bridge CFDictionaryRef) deleteQuery);
    
    NSMutableDictionary * query = [[NSMutableDictionary alloc] initWithDictionary:[MTRCSRKeys publicKeyParamsWithGroupId:grpId]];
    query[(__bridge NSString *) kSecValueData] = [keyData base64EncodedDataWithOptions:0];
    OSStatus pubKeyAddStatus = SecItemAdd((__bridge CFDictionaryRef) query, NULL);
    if (pubKeyAddStatus != errSecSuccess) {
        NSLog(@"Failed to store public key : %d", pubKeyAddStatus);
        return NULL;
    }
...
+ (SecKeyRef)loadCAPublicKeyWithGroupId:(NSString *)grpId {
    ...
    OSStatus status = SecItemCopyMatching((__bridge CFDictionaryRef) query, (CFTypeRef *) &keyDataRef);
    if (status != errSecSuccess || keyDataRef == nil) {
        NSLog(@"Did not find CA public key in the keychain for group: %@", grpId);
        return NULL;
    }

    NSLog(@"Found an existing CA public key in the keychain for group: %@", grpId);
    NSData * encodedKey = CFBridgingRelease(keyDataRef);

    NSData * keyData = [[NSData alloc] initWithBase64EncodedData:encodedKey options:0];
    if (keyData == nil) {
        NSLog(@"Could not base64-decode CA public key");
        return NULL;
    }
    ...
    if (error) {
        NSLog(@"Could not reconstruct public key %@", (__bridge NSError *) error);
        return NULL;
    }
